...
The customer broadcasts the funding transaction and waits for the network to confirm the transaction. Once the transaction is confirmed, the customer completes the initial commitment transaction and provides a signature to the merchant for his commitment transaction.

2.3 Initial Wallet Commitment
-------------
The initial commitment transaction is generated by the customer during the channel establishment but does not broadcast to the network. The customer's commitment transaction spends from the funding transaction with two outputs. One output ...
...
After each successful payment, the customer obtains a closing token for the updated balance of the channel and provides a revocation token to the merchant for the previous state along with the associated wallet public key.

2.4 Channel Closing
-------------
To close the channel, the customer can initiate by posting a commitment transaction that spends from the multi-signature transparent address with a witness that satisfies the script and the ``OP_BOLT`` opcode. The customer can then spend from a separate transaction that spends from the commitment transaction.
