...
The customer's commitment transaction is described below. ***Where is the customer closure token?***
...
* ``txouts``: 
  ...
  * ``to_customer``: a timelocked (using ``OP_CSV``) P2SH output sending funds back to the customer. So ``scriptPubKey`` is of the form ``0 <32-byte-hash>``.  
      ...
      - ``serializedScript``:
		...
		OP_IF		  
	  	  OP_2 <revocation-pubkey> <merch-pubkey> OP_2   ***Shouldn't this be revocation-token***
		OP_ELSE
...
***What does the merchant's commitment transaction look like?***
...
***I don't understand the above txouts... where is this coming from? we don't talk about nodes or peers or fee_satoshis or closing_signed***
...
This transaction has (up to 2) shielded inputs***again why only up to 2 inputs*** and 1 output to a 2-of-2 shielded address with the merchant public key. If an ``vBoltDescription`` field is added, then we could use it to store the channel parameters and the channel token for opening the channel.
...
This is a standard SegWit P2WSH transaction. Note that the witness and empty ``scriptSig`` are provided by a subsequent transaction that spends the funding transaction output. ***It's really confusing to include the witness like this, since the witness in the funding transaction itself is actually empty*** The ``scriptPubKey`` of the funding transaction indicates that a witness script should be provided with a given hash; the ``witnessScript`` (≤ 10,000 bytes) is popped off the initial witness stack of a spending transaction and the SHA256 of witnessScript must match the 32-byte hash of the following:

	2 <cust-pubkey> <merch-pubkey> 2 OP_CHECKMULTISIGVERIFY	
	OP_DUP OP_HASH160 <hash-of-channel-token> OP_EQUALVERIFY 

The channel token consists of the customer’s channel public key and wallet commitment from initializing the channel. The unique channel identifier is the hash of the channel public key. ***What is the channel public key?***

The ``<channel-token>`` is a serialized token with the following structure: (a) Initial wallet commitment, initial wallet public key and the channel public key. ***How can the channel token include the initial wallet public key? This is kept secret; again, what is the channel public key?***
...
* ``to_customer``: a timelocked (using ``OP_CSV``) version-0 P2WSH output sending funds back to the customer. So scriptPubKey is of the form ``0 <32-byte-hash>``. A customer node may create a transaction spending this output with:
  ...
  - ``witness script:``
	...
	OP_IF
	  ...
	  OP_2 <revocation-pubkey> <merch-pubkey> OP_2   ***What is revocation-pubkey?***
	OP_ELSE
	  # Customer must wait 
...
Or, if a revoked commitment transaction is available, the merchant may spend the output with the above witness script and witness stack:
	...
	<revocation-token> 1 <witnessScript>
			
To spend this***which output is this*** output, the merchant publishes a transaction with:
