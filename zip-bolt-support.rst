...
-------------
The initial commitment transaction is generated by the customer during the channel establishment but is not broadcast to the network. The customer's commitment transaction (below) contains an output that can be spent immediately by the merchant or can be spent by the customer after a timeout (or a certain number of blocks). This allows the merchant to see the parent transaction and spend the output with a revocation token if the customer posted an earlier state of the channel.  

The merchant's initial commitment transaction has two outputs that spends the full balance of the channel with a timeout and 
